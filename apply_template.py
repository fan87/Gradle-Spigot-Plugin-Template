...
def apply(original: str, repo_name: str) -> str:
    original = original.replace("spigot-plugin-template", generate_maven_name(repo_name))
    original = original.replace("spigotplugintemplate", repo_name.replace("-", "").lower())
    original = original.replace("SpigotPluginTemplate", repo_name.replace("-", ""))
    original = original.replace("Spigot-Plugin-Template", repo_name)
    return original
